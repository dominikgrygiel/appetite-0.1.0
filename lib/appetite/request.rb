class AppetiteRequest < Rack::Request # partially borrowed from Sinatra Framework
  ...
  ['', '_CHARSET', '_ENCODING', '_LANGUAGE', '_RANGES'].each do |field|
    define_method "accept#{field.downcase}?" do |value|
      @__appetite__accept_entries ||= {}
      @__appetite__accept_entries[field] ||= env[ENV__HTTP_ACCEPT + field]
      return unless @__appetite__accept_entries[field]
      @__appetite__accept_entries[field] =~ value.is_a?(Regexp) ? value : /#{value}/
    end
  end

  # Returns an array of acceptable media types for the response
  def accept
    @__appetite__accept ||= env[ENV__HTTP_ACCEPT].to_s.split(',').
